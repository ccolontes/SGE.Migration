...
```shell
dotnet new install Amantinband.SGE.Template

dotnet new clean-arch -o SGE
```
...
```shell
dotnet run --project src/SGE.Api
```
...
1. Deleting the subscription from the database and marking all reminders as deleted ([Subscriptions/Events/SubscriptionDeletedEventHandler.cs](src/SGE.Application/Subscriptions/Events/SubscriptionCanceledEventHandler.cs)])
1. Deleting all the reminders marked as deleted from the database ([Reminders/Events/ReminderDeletedEventHandler.cs](src/SGE.Application/Reminders/Events/ReminderDeletedEventHandler.cs)]
...
1. If `domain object B` needs to react to changes in `domain object A`, a [Domain Event](src/SGE.Domain/Common/IDomainEvent.cs) is added to `domain object A` alongside the changes.
1. Upon persisting `domain object A` changes to the database, the domain events are [extracted and added to a queue](src/SGE.Infrastructure/Common/Persistence/AppDbContext.cs) for offline processing:
    ```csharp
    ...
    ```
1. After the user receives a response, the [EventualConsistencyMiddleware](src/SGE.Infrastructure/Common/Middleware/EventualConsistencyMiddleware.cs) is invoked and processes the domain events:
    ```csharp
...
There is a simple background service that runs every minute and sends email reminders for all reminders that are due ([ReminderEmailBackgroundService](src/SGE.Infrastructure/Reminders/BackgroundServices/ReminderEmailBackgroundService.cs)):
...
```shell
dotnet user-secrets --project src/SGE.Api set EmailSettings:EnableEmailNotifications true
dotnet user-secrets --project src/SGE.Api set EmailSettings:DefaultFromEmail amantinband@gmail.com
dotnet user-secrets --project src/SGE.Api set EmailSettings:SmtpSettings:Server smtp-relay.brevo.com
dotnet user-secrets --project src/SGE.Api set EmailSettings:SmtpSettings:Port 587
dotnet user-secrets --project src/SGE.Api set EmailSettings:SmtpSettings:Username amantinband@gmail.com
dotnet user-secrets --project src/SGE.Api set EmailSettings:SmtpSettings:Password your-password
```
...
- [SGE](https://github.com/jasontaylordev/SGE) - An awesome clean architecture solution template by Jason Taylor
